...
void AMQPQueue::setHeaders(amqp_basic_properties_t * p) {
    ...
    if (p->_flags & AMQP_BASIC_HEADERS_FLAG) {
        ...
        for (int i = 0; i < max; i++) {
            ...
            name.assign((const char *)keyBytes.bytes, keyBytes.len);
            switch((p->headers).entries[i].value.kind) {
                case AMQP_FIELD_KIND_BOOLEAN:
                {
                    amqp_boolean_t value = (p->headers).entries[i].value.value.boolean;
                    pmessage->addHeader(name.c_str(), &value);
                    break;
                }
                case AMQP_FIELD_KIND_I8:
                {
                    uint8_t value = static_cast<uint8_t>((p->headers).entries[i].value.value.i8);
                    pmessage->addHeader(name.c_str(), &value);
                    break;
                }
                case AMQP_FIELD_KIND_U8:
                {
                    uint8_t value = (p->headers).entries[i].value.value.u8;
                    pmessage->addHeader(name.c_str(), &value);
                    break;
                }
                case AMQP_FIELD_KIND_I16:
                {
                    uint64_t value = static_cast<uint64_t>((p->headers).entries[i].value.value.i16);
                    pmessage->addHeader(name.c_str(), &value);
                    break;
                }
                case AMQP_FIELD_KIND_U16:
                {
                    uint64_t value = static_cast<uint64_t>((p->headers).entries[i].value.value.u16);
                    pmessage->addHeader(name.c_str(), &value);
                    break;
                }
                case AMQP_FIELD_KIND_I32:
                {
                    uint64_t value = static_cast<uint64_t>((p->headers).entries[i].value.value.i32);
                    pmessage->addHeader(name.c_str(), &value);
                    break;
                }
                case AMQP_FIELD_KIND_U32:
                {
                    uint64_t value = static_cast<uint64_t>((p->headers).entries[i].value.value.u32);
                    pmessage->addHeader(name.c_str(), &value);
                    break;
                }
                case AMQP_FIELD_KIND_I64:
                {
                    uint64_t value = static_cast<uint64_t>((p->headers).entries[i].value.value.i64);
                    pmessage->addHeader(name.c_str(), &value);
                    break;
                }
                case AMQP_FIELD_KIND_U64:
                {
                    uint64_t value = (p->headers).entries[i].value.value.u64;
                    pmessage->addHeader(name.c_str(), &value);
                    break;
                }
                case AMQP_FIELD_KIND_F32:
                {
                    double value = static_cast<double>((p->headers).entries[i].value.value.f32);
                    pmessage->addHeader(name.c_str(), &value);
                    break;
                }
                case AMQP_FIELD_KIND_F64:
                {
                    double value = (p->headers).entries[i].value.value.f32;
                    pmessage->addHeader(name.c_str(), &value);
                    break;
                }
                case AMQP_FIELD_KIND_UTF8:
                {
                    amqp_bytes_t value = (p->headers).entries[i].value.value.bytes;
                    pmessage->addHeader(name.c_str(), &value);
                    break;
                }
                ...
                default:
                    cerr << "unsupported type [" << (p->headers).entries[i].value.kind <<"]." << endl;
                    break;
            }
        }
    }
